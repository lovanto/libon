...
if(isset($_GET['logout'])){    
    $fp = fopen("log.html", 'a');
    fwrite($fp, "<div class='msgln'><i>". $_SESSION['user'] ." telah meninggalkan ruang obrolan.</i><br></div>");
    fclose($fp);
session_destroy();
...
if(isset($_POST['enter'])){
    if($_POST['user'] != ""){
        $_SESSION['user'] = stripslashes(htmlspecialchars($_POST['user']));
            $fp = fopen("log.html", 'a');
        fwrite($fp, "<div class='msgln'><i>". $_SESSION['user'] ." masuk ruang obrolan.</i><br></div>");
        fclose($fp);
    }
    else{
...
if(!isset($_SESSION['user'])){
    ...
    <form action="homeuser.php" method="post">
        ...
        <br><br>
        <input type="text" value="<?=$user?>" class="form-control" placeholder="Nama" name="user" id="user" style="width:90%;" readonly/>
        <br>
...
<div id="wrapper" class="data">
    <div id="menu">
        <p class="welcome">Selamat Datang, <b><?php echo $_SESSION['user']; ?></b></p>
...
$(document).ready(function(){
    $("#submitmsg").click(function(){    
        ...
        $.post("chatuser/post.php", {text: clientmsg});                
    ...
    $("#exit").click(function(){
        ...
        if(exit==true){window.location = 'homeuser.php?logout=true';}        
    });
});
</script>
<?php
